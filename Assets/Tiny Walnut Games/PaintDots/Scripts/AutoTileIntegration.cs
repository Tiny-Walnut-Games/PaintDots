...
namespace PaintDots.ECS.AutoTile
{
        ...
        private sealed class Baker : Baker<AutoTileAssetAuthoring>
        {
            public override void Bake(AutoTileAssetAuthoring authoring)
            {
                ...
                if (authoring.AutoTileScriptableObject != default)
                {
                    ...
                    AddComponent(entity, new AutoTileAsset(blobAsset, PaintDots.ECS.EntityConstants.InvalidEntity));
                }
            }
        }
    }
    ...
    // Uses central EntityConstants for sentinel values (no local EntitySentinels)
}