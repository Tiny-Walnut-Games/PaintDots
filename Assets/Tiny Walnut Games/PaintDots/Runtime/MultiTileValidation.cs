...
namespace PaintDots.Runtime.Validation
{
    /// <summary>
    ...
    public static class MultiTileValidation
    {
        ...
        public static bool ValidateStructureVisual()
        {
            ...
            const int expectedTileId = 77;
            var structureVisual = new StructureVisual(testEntity, testSize, expectedTileId);
            
            if (structureVisual.Prefab.Index != testEntity.Index) return false;
            if (!structureVisual.Size.Equals(testSize)) return false;
            if (structureVisual.PrimaryTileID != expectedTileId) return false;
