...
namespace PaintDots.Runtime.Examples
{
    /// <summary>
    ...
    public static class MultiTileExamples
    {
        ...
        /// <summary>
        /// Example: Create a structure paint command from a template's primary tile
        /// </summary>
        public static void CreateTemplatePaintCommand(EntityCommandBuffer.ParallelWriter ecb, int unfilteredChunkIndex, int2 origin, int primaryTileId, int2 footprintSize)
        {
            var command = PaintCommand.MultiTile(origin, primaryTileId, footprintSize);
    ...
    public partial struct ExampleLevelGeneratorSystem : ISystem
    {
        ...
        public void OnUpdate(ref SystemState state)
        {
            ...
            foreach (var (visual, footprint) in SystemAPI.Query<RefRO<StructureVisual>, RefRO<Footprint>>())
            {
                templateUsed = true;

                var spacing = new int2(footprint.ValueRO.Size.x + 2, footprint.ValueRO.Size.y + 2);
                var spawnOrigin = new int2(templateIndex * spacing.x, 0);

                MultiTileExamples.CreateTemplatePaintCommand(parallelWriter, 0, spawnOrigin, visual.ValueRO.PrimaryTileID, footprint.ValueRO.Size);
                templateIndex++;
            }
            ...
            if (!templateUsed)
            {
                ...
                MultiTileExamples.CreateExampleLevel(parallelWriter, 0, config);
            }
    ...
    public class MultiTileStructureAuthoring : UnityEngine.MonoBehaviour
    {
        ...
        public class Baker : Baker<MultiTileStructureAuthoring>
        {
            public override void Bake(MultiTileStructureAuthoring authoring)
            {
                ...
                // Add structure visual data that records the primary tile ID
                AddComponent(entity, new StructureVisual(entity, authoring.footprintSize, authoring.tileID));
